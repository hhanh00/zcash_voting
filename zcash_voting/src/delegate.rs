...
use crate::{
    ...
    types::{
        ...
        VotingHotkey, VotingHotkeyTarget, WitnessData,
    },
};
...
///
/// `Send + Sync` so callers can hold a provider across `.await` in async
/// hosts (e.g. FRB-generated bridge code).
pub trait BranchIdProvider: Send + Sync {
...
pub async fn ensure_round_context(
    voting_db: &VotingDb,
    network: Network,
    params: &RoundParams,
    round_name: &str,
    ...
    let state = voting_db
        .ensure_round_state(network, params, session_json)
        .await?;
...
/// Parameters for preparing one delegation bundle from caller-supplied inputs.
///
/// The caller (e.g. zkool) owns wallet storage and computes `round_note_infos`
/// and `witnesses` from its own database. Witnesses must be rooted at the
/// round snapshot's note commitment root (`round_params.nc_root`).
pub struct PrepareDelegationBundleWithInputsParams<'a> {
...
/// Prepares one delegation bundle from caller-supplied wallet data.
///
/// Callers should resolve [`DelegationLwdInputs`] via
/// [`gather_delegation_lwd_inputs`] and gather notes, witnesses, and keys from
/// their own wallet storage before calling. This validates/persists round
/// metadata, plans bundles from the supplied notes, and stores the supplied
/// witnesses in the voting database (idempotent). The caller is responsible
/// for checking that its wallet is synced to the round snapshot height.
///
/// # Errors
///
/// Returns an error if round context initialization, bundle preparation, or
/// witness persistence fails.
pub async fn prepare_delegation_bundle_with_inputs(
    voting_db: &VotingDb,
    params: PrepareDelegationBundleWithInputsParams<'_>,
) -> Result<PreparedDelegationBundle, VotingError> {
    let lwd = params.lwd;
    let session_json = params.session_json;
    if lwd.network != params.delegation_keys.network {
    ...
    let prepared = PreparedDelegationBundle {
        ...
        layout,
        bundle_note_infos,
        delegation_keys: params.delegation_keys,
        branch_id_provider,
        anchor_tree_state_bytes,
        network,
        round_name,
...
impl PreparedDelegationBundle {
    ...
    pub async fn submission(
        ...
        crate::delegate::submission(voting_db, &self.round_id, self.bundle_index, signer).await
    }
    ...
    pub async fn signed_bundle(
        ...
        let submission = self.submission(voting_db, signer).await?;
    ...
    pub async fn keystone_request(
        &self,
        voting_db: &VotingDb,
        stages: &dyn DelegationProgressReporter,
    ) -> Result<KeystoneSigningRequest, VotingError> {
        let setup = self.setup(voting_db, stages).await?;
...
pub async fn setup(
    ...
    let pczt = db
        .build_governance_pczt(round_id, bundle_index, notes, keys, consensus_branch_id)
        .await?;
...
pub async fn prove<P: crate::pir::PirProofSource>(
    ...
    let proof = db
        .build_and_prove_delegation(round_id, bundle_index, notes, keys, pir_client, stages)
        .await?;
...
pub async fn record_van_position(
    db: &VotingDb,
    round_id: &str,
    bundle_index: u32,
    position: u32,
